    ...
    return( 
        <section className={`${styles.spotlight_wrap} py-6 lg:py-14 px-0 md:px-0 w-full`}>
            <div className="inner_section w-full lg:max-w-5xl m-auto px-6 lg:px-10">
                ...
                <div className={`${styles.project_card_wrap} grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-5 lg:py-6 lg:px-5`}>
                    {workPosts.map((workPost) => (
                    <article key={workPost._id} className={`${styles.project_card} flex flex-col justify-between`}>

                        <Image src={workPost.featured.asset.url} className={`${styles.spotlight_feat_image} mb-3 w-full`} alt={workPost.featured.alt} width={358} height={122} />
                        ...
                        <Link href={`/projekt/${workPost.slug.current}`} className="link_button py-2 px-0 w-full">Gå till projektet</Link>
                    </article>
                    ))}
                </div>
